...
${o}
${s}
</head><body>${e}</body></html>`),a.close();let c=()=>{r.remove()};i.onafterprint=c,setTimeout(()=>{r.parentNode&&c()},2e3),i.focus(),i.print()}htmlToPlainText(e){return Qe(e)}toHtml(e,t){if(e.endsWith(`.txt`))return $e(t);let n=Ze(t);return this.normalizeTableCells(n)}normalizeTableCells(e){let t=new DOMParser().parseFromString(e,`text/html`);return t.querySelectorAll(`td, th`).forEach(e=>{e.style.verticalAlign||(e.style.verticalAlign=`middle`)}),t.body.innerHTML}},Tt=(e,t)=>t.some(t=>e instanceof t),Et,Dt;function Ot(){return Et||=[IDBDatabase,IDBObjectStore,IDBIndex,IDBCursor,IDBTransaction]}function kt(){return Dt||=[IDBCursor.prototype.advance,IDBCursor.prototype.continue,IDBCursor.prototype.continuePrimaryKey]}var At=new WeakMap,jt=new WeakMap,L=new WeakMap;function Mt(e){let t=new Promise((t,n)=>{let r=()=>{e.removeEventListener(`success`,i),e.removeEventListener(`error`,a)},i=()=>{t(Rt(e.result)),r()},a=()=>{n(e.error),r()};e.addEventListener(`success`,i),e.addEventListener(`error`,a)});return L.set(t,e),t}function Nt(e){if(At.has(e))return;let t=new Promise((t,n)=>{let r=()=>{e.removeEventListener(`complete`,i),e.removeEventListener(`error`,a),e.removeEventListener(`abort`,a)},i=()=>{t(),r()},a=()=>{n(e.error||new DOMException(`AbortError`,`AbortError`)),r()};e.addEventListener(`complete`,i),e.addEventListener(`error`,a),e.addEventListener(`abort`,a)});At.set(e,t)}var Pt={get(e,t,n){if(e instanceof IDBTransaction){if(t===`done`)return At.get(e);if(t===`store`)return n.objectStoreNames[1]?void 0:n.objectStore(n.objectStoreNames[0])}return Rt(e[t])},set(e,t,n){return e[t]=n,!0},has(e,t){return e instanceof IDBTransaction&&(t===`done`||t===`store`)?!0:t in e}};function Ft(e){Pt=e(Pt)}function It(e){return kt().includes(e)?function(...t){return e.apply(zt(this),t),Rt(this.request)}:function(...t){return Rt(e.apply(zt(this),t))}}function Lt(e){return typeof e==`function`?It(e):(e instanceof IDBTransaction&&Nt(e),Tt(e,Ot())?new Proxy(e,Pt):e)}function Rt(e){if(e instanceof IDBRequest)return Mt(e);if(jt.has(e))return jt.get(e);let t=Lt(e);return t!==e&&(jt.set(e,t),L.set(t,e)),t}var zt=e=>L.get(e);function Bt(e,t,{blocked:n,upgrade:r,blocking:i,terminated:a}={}){let o=indexedDB.open(e,t),s=Rt(o);return r&&o.addEventListener(`upgradeneeded`,e=>{r(Rt(o.result),e.oldVersion,e.newVersion,Rt(o.transaction),e)}),n&&o.addEventListener(`blocked`,e=>n(e.oldVersion,e.newVersion,e)),s.then(e=>{a&&e.addEventListener(`close`,()=>a()),i&&e.addEventListener(`versionchange`,e=>i(e.oldVersion,e.newVersion,e))}).catch(()=>{}),s}var Vt=[`get`,`getKey`,`getAll`,`getAllKeys`,`count`],Ht=[`put`,`add`,`delete`,`clear`],Ut=new Map;function Wt(e,t){if(!(e instanceof IDBDatabase&&!(t in e)&&typeof t==`string`))return;if(Ut.get(t))return Ut.get(t);let n=t.replace(/FromIndex$/,``),r=t!==n,i=Ht.includes(n);if(!(n in(r?IDBIndex:IDBObjectStore).prototype)||!(i||Vt.includes(n)))return;let a=async function(e,...t){let a=this.transaction(e,i?`readwrite`:`readonly`),o=a.store;return r&&(o=o.index(t.shift())),(await Promise.all([o[n](...t),i&&a.done]))[0]};return Ut.set(t,a),a}Ft(e=>({...e,get:(t,n,r)=>Wt(t,n)||e.get(t,n,r),has:(t,n)=>!!Wt(t,n)||e.has(t,n)}));var Gt=[`continue`,`continuePrimaryKey`,`advance`],Kt={},qt=new WeakMap,Jt=new WeakMap,Yt={get(e,t){if(!Gt.includes(t))return e[t];let n=Kt[t];return n||=Kt[t]=function(...e){qt.set(this,Jt.get(this)[t](...e))},n}};async function*Xt(...e){let t=this;if(t instanceof IDBCursor||(t=await t.openCursor(...e)),!t)return;t=t;let n=new Proxy(t,Yt);for(Jt.set(n,t),L.set(n,zt(t));t;)yield n,t=await(qt.get(n)||t.continue()),qt.delete(n)}function Zt(e,t){return t===Symbol.asyncIterator&&Tt(e,[IDBIndex,IDBObjectStore,IDBCursor])||t===`iterate`&&Tt(e,[IDBIndex,IDBObjectStore])}Ft(e=>({...e,get(t,n,r){return Zt(t,n)?Xt:e.get(t,n,r)},has(t,n){return Zt(t,n)||e.has(t,n)}}));var Qt=`poa-editor-autosave`,$t=`snapshots`,en=10,tn=300*1e3,nn=class{timer=null;intervalMs;constructor(e=tn){this.intervalMs=e}start(e){this.stop(),this.timer=setInterval(()=>{this.saveNow(e())},this.intervalMs)}stop(){this.timer!==null&&(clearInterval(this.timer),this.timer=null)}isRunning(){return this.timer!==null}async saveNow(e){let t=(await this.openDB()).transaction($t,`readwrite`);await t.store.put({html:e,savedAt:Date.now()});let n=await t.store.getAllKeys();n.length>en&&await t.store.delete(n[0]),await t.done,N.emit(P.AUTOSAVE_SAVED,{savedAt:Date.now()})}async listSnapshots(){let e=await this.openDB(),[t,n]=await Promise.all([e.getAllKeys($t),e.getAll($t)]);return n.map((e,n)=>({id:t[n],html:e.html,savedAt:e.savedAt})).reverse()}async restoreSnapshot(e){return(await(await this.openDB()).get($t,e))?.html??null}async clearAll(){await(await this.openDB()).clear($t)}async openDB(){return Bt(Qt,1,{upgrade(e){e.objectStoreNames.contains($t)||e.createObjectStore($t,{autoIncrement:!0})}})}},rn=`poa-settings`,an={fontFamily:`맑은 고딕`,fontSize:11,lineHeight:1.5,autoSaveEnabled:!0,autoSaveInterval:5,spellCheck:!1,changeWarning:!0};function on(){try{let e=localStorage.getItem(rn);return e?{...an,...JSON.parse(e)}:{...an}}catch{return{...an}}}function sn(e){try{localStorage.setItem(rn,JSON.stringify(e))}catch{}}var cn=[`맑은 고딕`,`나눔고딕`,`나눔명조`,`굴림`,`돋움`,`바탕`,`궁서`,`Arial`,`Georgia`,`Times New Roman`,`Verdana`],ln=[8,9,10,11,12,14,16,18,20,24,28,32],un=[1,1.2,1.4,1.5,1.6,1.8,2,2.5],dn=[1,2,3,5,10,15,30],fn=`
:host { display: none; }
...
`;function pn(e,t,n){return`<option value="${e}"${n?` selected`:``}>${t}</option>`}var mn=class extends HTMLElement{shadow;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${fn}</style>
    ...
    <div class="body">
      <div class="section">
        ...
        <div class="row">
          ...
          <div class="row-ctrl">
            <select id="sel-font-family">${cn.map(e=>pn(e,e,!1)).join(``)}</select>
          </div>
        </div>
        <div class="row">
          <span class="row-label">글자 크기</span>
          <div class="row-ctrl">
            <select id="sel-font-size">${ln.map(e=>pn(String(e),`${e}pt`,!1)).join(``)}</select>
          </div>
        </div>
        <div class="row">
          <span class="row-label">줄 간격</span>
          <div class="row-ctrl">
            <select id="sel-line-height">${un.map(e=>pn(String(e),String(e),!1)).join(``)}</select>
          </div>
        </div>
      </div>
      <div class="section">
        ...
        <div class="row" id="row-interval">
          ...
          <div class="row-ctrl">
            <select id="sel-interval">${dn.map(e=>pn(String(e),`${e}분`,!1)).join(``)}</select>
          </div>
        </div>
      </div>
      <div class="section">
...
<div style="display:flex;justify-content:flex-end;gap:8px;margin-top:16px;">
  ...
  <button id="cp-ok" style="height:28px;padding:0 14px;border:1px solid #1565c0;border-radius:3px;background:#1565c0;color:#fff;cursor:pointer;">적용</button>
    ...
    <line x1="3"  y1="25" x2="37" y2="25" stroke="#bbb" stroke-width="1"/>`),"border-thick":Mn(`
    ...
    <line x1="2"  y1="25" x2="38" y2="25" stroke="#aaa" stroke-width="1"/>`),"header-light":Mn(`
    ...
    <line x1="25" y1="3" x2="25" y2="37" stroke="#ccc" stroke-width="1"/>`),"header-dark":Mn(`
    ...
    <line x1="25" y1="3" x2="25" y2="37" stroke="#888" stroke-width="1"/>`),"stripe-light":Mn(`
    ...
    <line x1="25" y1="3" x2="25" y2="37" stroke="#ddd" stroke-width="1"/>`),"stripe-dark":Mn(`
    ...
    <line x1="14" y1="3" x2="14" y2="37" stroke="#bbb" stroke-width="1"/>
    <line x1="25" y1="3" x2="25" y2="37" stroke="#bbb" stroke-width="1"/>`),borderless:Mn(`
    ...
    <line x1="3"  y1="25" x2="37" y2="25" stroke="#e0e0e0" stroke-width="1" stroke-dasharray="2,2"/>`),dashed:Mn(`
    ...
    <line x1="3"  y1="25" x2="37" y2="25" stroke="#999" stroke-width="1" stroke-dasharray="3,2"/>`)},Pn=[{id:`border-all`,label:`전체 테두리`,icon:Nn[`border-all`],baseOptions:{width:`100%`,border:1,borderColor:`#cccccc`,headerPosition:`none`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:1px solid #ccc;${On}`}},{id:`border-thick`,label:`굵은 외부`,icon:Nn[`border-thick`],baseOptions:{width:`100%`,border:1,borderColor:`#aaaaaa`,headerPosition:`none`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:1px solid #aaa;${On}`;e.style.boxShadow=`0 0 0 2px #333333`}},{id:`header-light`,label:`헤더 강조`,icon:Nn[`header-light`],baseOptions:{width:`100%`,border:1,borderColor:`#cccccc`,headerPosition:`top`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:1px solid #ccc;${On}`;for(let t of An(e))t.style.cssText=`border:1px solid #ccc;${On}background:#f0f0f0;font-weight:bold;`}},{id:`header-dark`,label:`헤더 진한`,icon:Nn[`header-dark`],baseOptions:{width:`100%`,border:1,borderColor:`#aaaaaa`,headerPosition:`top`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:1px solid #aaa;${On}`;for(let t of An(e))t.style.cssText=`border:1px solid #555;${On}background:#333333;color:#ffffff;font-weight:bold;`}},{id:`stripe-light`,label:`줄무늬`,icon:Nn[`stripe-light`],baseOptions:{width:`100%`,border:1,borderColor:`#dddddd`,headerPosition:`none`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:1px solid #ddd;${On}`;jn(e).forEach((e,t)=>{let n=t%2==0?`#f9f9f9`:``;for(let t of Array.from(e.querySelectorAll(`td,th`)))t.style.backgroundColor=n})}},{id:`stripe-dark`,label:`진한 줄무늬`,icon:Nn[`stripe-dark`],baseOptions:{width:`100%`,border:1,borderColor:`#bbbbbb`,headerPosition:`none`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:1px solid #bbb;${On}`;jn(e).forEach((e,t)=>{let n=t%2==0?`#e0e0e0`:``;for(let t of Array.from(e.querySelectorAll(`td,th`)))t.style.backgroundColor=n})}},{id:`borderless`,label:`테두리 없음`,icon:Nn.borderless,baseOptions:{width:`100%`,border:0,headerPosition:`none`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:none;${On}`;e.removeAttribute(`border`)}},{id:`dashed`,label:`점선 테두리`,icon:Nn.dashed,baseOptions:{width:`100%`,border:1,borderColor:`#999999`,headerPosition:`none`},apply:e=>{for(let t of kn(e))t.style.cssText=`border:1px dashed #999;${On}`}}];function Fn(e,t){Pn.find(t=>t.id===e)?.apply(t)}var In=new Set([`http:`,`https:`,`mailto:`,`tel:`]);function Ln(e){if(e.startsWith(`#`))return e.length>1;try{let t=new URL(e);return In.has(t.protocol)}catch{return!1}}var Rn=class{root;savedRange=null;constructor(e){this.root=e}saveSelection(){let e=this.root.ownerDocument.getSelection();e&&e.rangeCount>0&&(this.savedRange=e.getRangeAt(0).cloneRange())}insertLink(e){if(!Ln(e.href))throw Error(`유효하지 않은 URL입니다.`);let t=this.root.ownerDocument.createElement(`a`);t.href=e.href,e.title&&(t.title=e.title),e.target===`_blank`&&(t.target=`_blank`,t.rel=`noopener noreferrer`),t.textContent=this.savedRange?.toString().trim()||e.text.trim()||e.href,e.target===`_self`&&(t.target=`_self`),this.insertNode(t)}updateLink(e,t){if(t.href!==void 0){if(!Ln(t.href))throw Error(`유효하지 않은 URL입니다.`);e.href=t.href}t.text!==void 0&&(e.textContent=t.text),t.title!==void 0&&(e.title=t.title),t.target!==void 0&&(t.target===`_blank`?(e.target=`_blank`,e.rel=`noopener noreferrer`):(e.target=`_self`,e.removeAttribute(`rel`)))}removeLink(e){let t=e.parentNode;if(t){for(;e.firstChild;)t.insertBefore(e.firstChild,e);t.removeChild(e)}}getFocusedAnchor(){let e=this.root.ownerDocument.getSelection();if(!e||e.rangeCount===0)return null;let t=e.getRangeAt(0).startContainer;for(;t&&t!==this.root;){if(t.nodeType===Node.ELEMENT_NODE&&t.tagName===`A`)return t;t=t.parentNode}return null}insertNode(e){let t=this.root.ownerDocument,n=t.getSelection(),r;this.savedRange?(r=this.savedRange.cloneRange(),this.savedRange=null):n&&n.rangeCount>0?r=n.getRangeAt(0):(r=t.createRange(),r.selectNodeContents(this.root),r.collapse(!1)),r.deleteContents(),r.insertNode(e),r.setStartAfter(e),r.collapse(!0),n?.removeAllRanges(),n?.addRange(r)}};function zn(){let e=new Uint8Array(6);return crypto.getRandomValues(e),Array.from(e,e=>e.toString(16).padStart(2,`0`)).join(``)}var Bn=class{root;savedRange=null;constructor(e){this.root=e}saveSelection(){let e=this.root.ownerDocument.getSelection();e&&e.rangeCount>0&&(this.savedRange=e.getRangeAt(0).cloneRange())}insert(e){let t=`bm-${zn()}`,n=this.root.ownerDocument.createElement(`a`);return n.id=t,n.setAttribute(`name`,t),n.className=`poa-bookmark`,n.setAttribute(`data-label`,e),n.title=e||t,n.textContent=`[${e||t}]`,n.contentEditable=`false`,this.insertAtRange(n),t}getAll(){return Array.from(this.root.querySelectorAll(`a.poa-bookmark[id^="bm-"]`)).map(e=>({id:e.id,label:e.getAttribute(`data-label`)??e.id,element:e}))}update(e,t){let n=this.root.querySelector(`a[id="${e}"]`);if(!n)throw Error(`책갈피 '${e}'를 찾을 수 없습니다.`);n.setAttribute(`data-label`,t),n.title=t,n.textContent=`[${t||e}]`}remove(e){this.root.querySelector(`a[id="${e}"]`)?.remove()}insertAtRange(e){let t=this.root.ownerDocument,n=t.getSelection(),r;this.savedRange?(r=this.savedRange.cloneRange(),this.savedRange=null):n&&n.rangeCount>0?r=n.getRangeAt(0):(r=t.createRange(),r.selectNodeContents(this.root),r.collapse(!1)),r.collapse(!0),r.insertNode(e),r.setStartAfter(e),r.collapse(!0),n?.removeAllRanges(),n?.addRange(r)}},Vn=[{dir:`nw`,cursor:`nw-resize`,top:`0`,left:`0`},{dir:`n`,cursor:`n-resize`,top:`0`,left:`50%`},{dir:`ne`,cursor:`ne-resize`,top:`0`,left:`100%`},{dir:`e`,cursor:`e-resize`,top:`50%`,left:`100%`},{dir:`se`,cursor:`se-resize`,top:`100%`,left:`100%`},{dir:`s`,cursor:`s-resize`,top:`100%`,left:`50%`},{dir:`sw`,cursor:`sw-resize`,top:`100%`,left:`0`},{dir:`w`,cursor:`w-resize`,top:`50%`,left:`0`}],Hn=20,Un=class{root;cb;overlay=null;activeImg=null;dragging=!1;dragDir=`se`;dragStartX=0;dragStartY=0;dragStartW=0;dragStartH=0;constructor(e,t={}){this.root=e,this.cb=t}attach(){this.root.style.position||(this.root.style.position=`relative`),this.root.addEventListener(`click`,this.onRootClick,!0),this.root.addEventListener(`contextmenu`,this.onContextMenu),this.root.addEventListener(`input`,this.onContentInput),document.addEventListener(`mousemove`,this.onMouseMove),document.addEventListener(`mouseup`,this.onMouseUp)}detach(){this.deactivate(),this.root.removeEventListener(`click`,this.onRootClick,!0),this.root.removeEventListener(`contextmenu`,this.onContextMenu),this.root.removeEventListener(`input`,this.onContentInput),document.removeEventListener(`mousemove`,this.onMouseMove),document.removeEventListener(`mouseup`,this.onMouseUp)}getActiveImage(){return this.activeImg}syncOverlay(){this.updatePos()}deactivate(){this.overlay?.remove(),this.overlay=null,this.activeImg&&(this.activeImg=null,this.cb.onDeactivate?.())}activate(e){this.activeImg!==e&&(this.overlay?.remove(),this.activeImg=e,this.createOverlay(),this.cb.onActivate?.(e))}createOverlay(){if(!this.activeImg)return;let e=document.createElement(`div`);e.className=`poa-img-resize-overlay`,e.dataset.poaTemp=`true`,e.style.cssText=`position:absolute;border:2px solid #0078d7;pointer-events:none;z-index:10;box-sizing:border-box;`;for(let t of Vn){let n=document.createElement(`div`);n.dataset.dir=t.dir,n.style.cssText=`position:absolute;width:8px;height:8px;background:#0078d7;border:1px solid #fff;border-radius:1px;box-sizing:border-box;cursor:${t.cursor};top:${t.top};left:${t.left};transform:translate(-50%,-50%);pointer-events:all;z-index:11;`,n.addEventListener(`mousedown`,this.onHandleMouseDown),e.appendChild(n)}this.root.appendChild(e),this.overlay=e,this.updatePos()}updatePos(){if(!this.overlay||!this.activeImg)return;let e=this.activeImg.getBoundingClientRect(),t=this.root.getBoundingClientRect(),n=e.top-t.top+this.root.scrollTop,r=e.left-t.left+this.root.scrollLeft;Object.assign(this.overlay.style,{top:`${n}px`,left:`${r}px`,width:`${e.width}px`,height:`${e.height}px`})}onRootClick=e=>{let t=e.target;this.overlay?.contains(t)||(t.tagName===`IMG`?this.activate(t):this.deactivate())};onContextMenu=e=>{let t=e.target;if(t.tagName!==`IMG`)return;e.preventDefault();let n=t;this.activate(n),this.cb.onContextMenu?.(n,e.clientX,e.clientY)};onContentInput=()=>{this.activeImg&&!this.root.contains(this.activeImg)&&this.deactivate()};onHandleMouseDown=e=>{if(!this.activeImg)return;let t=e.currentTarget.dataset.dir;e.preventDefault(),e.stopPropagation();let n=this.activeImg.getBoundingClientRect();this.dragging=!0,this.dragDir=t,this.dragStartX=e.clientX,this.dragStartY=e.clientY,this.dragStartW=n.width,this.dragStartH=n.height,document.body.style.cursor=e.currentTarget.style.cursor,document.body.style.userSelect=`none`};onMouseMove=e=>{if(!this.dragging||!this.activeImg)return;let t=e.clientX-this.dragStartX,n=e.clientY-this.dragStartY,r=this.dragDir,i=this.root.clientWidth,a=this.dragStartW,o=this.dragStartH;(r===`e`||r===`ne`||r===`se`)&&(a=this.dragStartW+t),(r===`w`||r===`nw`||r===`sw`)&&(a=this.dragStartW-t),(r===`s`||r===`se`||r===`sw`)&&(o=this.dragStartH+n),(r===`n`||r===`ne`||r===`nw`)&&(o=this.dragStartH-n);let s=r===`nw`||r===`ne`||r===`sw`||r===`se`;if(e.shiftKey&&s&&this.dragStartH>0){let e=this.dragStartW/this.dragStartH;Math.abs(t)>=Math.abs(n)?o=a/e:a=o*e}a=Math.max(Hn,Math.min(i,a)),o=Math.max(Hn,o),this.activeImg.style.width=`${Math.round(a)}px`,this.activeImg.style.height=`${Math.round(o)}px`,this.updatePos(),this.cb.onResize?.(this.activeImg)};onMouseUp=()=>{this.dragging&&(this.dragging=!1,document.body.style.cursor=``,document.body.style.userSelect=``,this.activeImg&&(this.updatePos(),this.cb.onResizeEnd?.()))}},Wn=794,Gn=1123,Kn=24,qn={top:60,right:80,bottom:60,left:80},Jn=`poa-print-styles`,Yn=class{container=null;mount(e,t,n=[],r=qn){this.container=e,e.innerHTML=``;let i=document.createElement(`div`);i.className=`poa-page-main`,i.style.cssText=`flex:1;overflow-y:auto;padding:20px;display:flex;flex-direction:column;align-items:center;background:#e8e8e8;`,this.injectPrintStyles(r);let a=this.splitByPageBreaks(t);for(let e of a)i.appendChild(this.buildPage(e,r));e.appendChild(i),n.length>0&&e.appendChild(this.buildSidebar(n))}unmount(){this.container&&=(this.container.innerHTML=``,null),document.getElementById(Jn)?.remove()}getPageCount(e){return this.splitByPageBreaks(e).length}buildPage(e,t){let n=document.createElement(`div`);return n.className=`poa-a4-page`,n.style.cssText=[`width:${Wn}px`,`min-height:${Gn}px`,`background:#fff`,`box-shadow:0 2px 8px rgba(0,0,0,.2)`,`padding:${t.top}px ${t.right}px ${t.bottom}px ${t.left}px`,`box-sizing:border-box`,`margin-bottom:20px`,`position:relative`,`font-size:14px`,`line-height:1.6`].join(`;`),n.innerHTML=e,n}buildSidebar(e){let t=document.createElement(`div`);t.className=`poa-page-sidebar`,t.style.cssText=[`width:180px`,`flex-shrink:0`,`overflow-y:auto`,`background:#fafafa`,`border-left:1px solid #ddd`,`padding:12px`].join(`;`);let n=document.createElement(`div`);n.style.cssText=`font-size:12px;font-weight:600;color:#555;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid #eee;`,n.textContent=`책갈피`,t.appendChild(n);for(let n of e){let e=document.createElement(`a`);e.href=`#${n.id}`,e.textContent=n.label,e.style.cssText=`display:block;font-size:12px;color:#1976d2;text-decoration:none;padding:5px 0;border-bottom:1px solid #f0f0f0;`,e.addEventListener(`mouseenter`,()=>{e.style.textDecoration=`underline`}),e.addEventListener(`mouseleave`,()=>{e.style.textDecoration=`none`}),t.appendChild(e)}return t}splitByPageBreaks(e){let t=e.split(/<hr[^>]*class="[^"]*poa-page-break[^"]*"[^>]*\/?>|<div[^>]*class="[^"]*poa-page-break[^"]*"[^>]*>[\s\S]*?<\/div>/gi);return t.length>0?t:[e]}injectPrintStyles(e){if(document.getElementById(Jn))return;let t=document.createElement(`style`);t.id=Jn,t.textContent=`
@media print {
...
<div id="pf-header" style="padding:10px 12px 10px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #F1F5F9;gap:8px;cursor:move;">
  <div style="display:flex;align-items:center;gap:8px;min-width:0;">
    <span style="display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;background:linear-gradient(135deg,#3B82F6,#2563EB);border-radius:7px;color:#fff;font-size:11px;font-weight:700;flex-shrink:0;">${gi[u]??`T`}</span>
...
`,ia=class extends HTMLElement{shadow;targetCell=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${ra}</style>
...
`,oa=class extends HTMLElement{shadow;activeTab=`video`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${aa}</style>`}open(e=`video`){this.activeTab=e,this.setAttribute(`open`,``),this.render()}close(){this.removeAttribute(`open`)}render(){this.shadow.innerHTML=`<style>${aa}</style>
<div class="overlay" id="overlay">
  ...
  </svg>`},ca={text:`텍스트`,textarea:`여러 줄`,checkbox:`체크박스`,radio:`라디오`,select:`목록 선택`,button:`버튼`,date:`날짜 선택`},la=[`text`,`textarea`,`checkbox`,`radio`,`select`,`button`,`date`],ua=`
...
`;function K(e,t,n=``){return`<div class="field">
  ...
  </div>`}function q(e,t=``,n=`text`){return`<input type="${n}" id="${e}" placeholder="${t}">`}function da(e,t){return`<select id="${e}">${t.map(([e,t])=>`<option value="${e}">${t}</option>`).join(``)}</select>`}var fa=class extends HTMLElement{shadow;activeType=`text`;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${ua}</style>${this._tpl()}`,this._buildTypeCards(),this._bindEvents(),this._setType(`text`)}open(e){let t=this.shadow.getElementById(`dlg-title`),n=this.shadow.getElementById(`btn-confirm`);t.textContent=e?`입력 요소 편집`:`입력 요소 추가`,n.textContent=e?`수정하기`:`추가하기`,e?(this._setType(e.type),this._fillFields(e)):(this._setType(`text`),this._clearFields()),this.setAttribute(`open`,``),this.shadow.getElementById(`f-label`)?.focus()}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    ...
    <div class="adv-body" data-adv-body>${e}</div>`}_secText(){return`<div class="specific-section" id="sec-text">
      ...
      ${this._adv(`
        <div class="field">
          ...
          ${da(`f-btn-style`,[[`default`,`기본 (회색)`],[`primary`,`강조 (파랑)`],[`danger`,`경고 (빨강)`]])}
        </div>
...
`;function J(e,t){return e.replace(/width="24"/,`width="${t}"`).replace(/height="24"/,`height="${t}"`)}function Y(e){return J(e,16)}function X(e){return J(e,14)}function cs(e){return J(e,12)}var Z={bold:Y(Ta),italic:Y(io),underline:Y(is),strike:Y(Go),undo:Y(as),redo:Y(jo),alignLeft:Y(ts),alignCenter:Y(Qo),alignRight:Y($o),alignJustify:Y(es),indent:Y(po),outdent:Y(fo),foreColor:Y(Ca),backColor:Y(Qa),lineHeight:Y(wa),letterSpacing:Y(So),fileNew:Y(Ua),fileOpen:Y(qa),save:Y(Fo),fileSaveAs:Y(Ha),print:Y(ko),settings:Y(zo),clock:Y(La),history:Y($a),cut:Y(Io),copy:Y(Ba),paste:Y(Ia),pastePlain:Y(Fa),selectAll:Y(Wo),search:Y(Lo),imageEdit:Y(eo),image:Y(to),images:Y(no),videoTag:Y(Ka),embedVideo:Y(Na),link:Y(uo),linkOff:Y(lo),bookmark:Y(Da),signature:Y(Eo),emoji:Y(Ho),tooltip:Y(_o),tooltipList:Y(vo),calendar:Y(Oa),hr:Y(bo),symbol:Y(Co),youtube:Y(xo),pageBreak:Y(Ro),template:Y(co),formField:Y(Ao),viewDesign:Y(oo),viewHtml:Y(Ra),viewPreview:Y(Va),viewText:Y(rs),viewPage:Y(Wa),fullscreen:Y(go),fullscreenExit:Y(yo),maximize2_14:X(go),minimize2_14:X(yo),ruler:Y(Po),grid:Y(Ya),hiddenBorder:Y(Wo),table:Y(Zo),tableProps:Y(Xo),cellProps:Y(so),mergeCells:Y(Jo),splitCell:Y(Yo),rowAbove:Y(Sa),rowBelow:Y(ya),colLeft:Y(ba),colRight:Y(xa),rowDelete:Y(No),colDelete:Y(za),tableDelete:Y(ns),painterCopy:Y(wo),painterPaste:Y(To),formatClear:Y(Mo),ul:Y(ho),ol:Y(mo),sup:Y(qo),sub:Y(Ko),a11y:Y(va),privacy:Y(Bo),form:Y(Uo),calc:Y(Vo),userMode:Y(os),shortcuts:Y(ao),guide:Y(Ea),about:Y(ro),folder:Y(Ja),folderOpen:Y(qa),file:Y(Ga),fileText16:Y(Wa),users16:Y(ss),user16:Y(os),userCircle:Y(Pa),plus:Y(Oo),pencil:Y(Do),trash:Y(ns),link16:Y(uo),chevronRight:Y(Aa),chevronDown:Y(ka),folder14:X(Ja),folderOpen14:X(qa),file14:X(Ga),fileText14:X(Wa),users14:X(ss),user14:X(os),grip14:X(Xa),chevronRight12:cs(Aa),chevronDown12:cs(ka),users12:cs(ss),user12:cs(os),fieldText:X(rs),fieldNumber:X(Za),fieldDate:X(Oa),layoutTemplate48:J(co,48)},ls={"file:new":`fileNew`,"file:open":`fileOpen`,"file:save":`save`,"file:saveas":`fileSaveAs`,"file:print":`print`,"file:history":`history`,settings:`settings`,"edit:cut":`cut`,"edit:copy":`copy`,"edit:paste":`paste`,"edit:paste-plain":`pastePlain`,"edit:select-all":`selectAll`,"find-replace":`search`,"edit:image-edit":`imageEdit`,image:`image`,"insert:multi-image":`images`,"insert:video":`videoTag`,"insert:embed":`embedVideo`,"insert:link":`link`,"insert:link-remove":`linkOff`,"insert:bookmark":`bookmark`,"insert:signature":`signature`,"insert:emoji":`emoji`,"insert:tooltip":`tooltip`,"insert:tooltip-list":`tooltipList`,"insert:datetime":`calendar`,"insert:hr":`hr`,"insert:symbol":`symbol`,"insert:pagebreak":`pageBreak`,"misc:template":`template`,"view:design":`viewDesign`,"view:html":`viewHtml`,"view:preview":`viewPreview`,"view:text":`viewText`,"view:page":`viewPage`,"view:fullscreen":`fullscreen`,"view:ruler":`ruler`,"view:grid":`grid`,"view:hidden-border":`hiddenBorder`,table:`table`,"table:table-props":`tableProps`,"table:cell-props":`cellProps`,"table:merge":`mergeCells`,"table:split-cell":`splitCell`,"table:row-above":`rowAbove`,"table:row-below":`rowBelow`,"table:col-left":`colLeft`,"table:col-right":`colRight`,"table:row-delete":`rowDelete`,"table:col-delete":`colDelete`,"table:delete":`tableDelete`,"table:align-left":`alignLeft`,"table:align-center":`alignCenter`,"table:align-right":`alignRight`,"format:painter-copy":`painterCopy`,"format:painter-paste":`painterPaste`,"format:clear":`formatClear`,"format:ul":`ul`,"format:ol":`ol`,"format:sup":`sup`,"format:sub":`sub`,"misc:a11y":`a11y`,"misc:privacy":`privacy`,"misc:form":`form`,"misc:calc":`calc`,"misc:user-mode":`userMode`,"help:shortcuts":`shortcuts`,"help:guide":`guide`,"help:about":`about`};function us(e){return!(e.isTemp||!e.name||e.name.trim().length===0||e.name.startsWith(`임시_`)||e.name.startsWith(`preview_`)||e.name.startsWith(`__`)||e.type===`template`&&e.content==null)}var ds=`
*, *::before, *::after { box-sizing: border-box; }
...
`,Ds=class extends HTMLElement{shadow;inserter=new li;activeCatId=oi[0].id;connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Es}</style>
<div class="dlg">
...
</div>`,this._buildCatBar(),this._bind()}open(){this._renderCategory(this.activeCatId),this.setAttribute(`open`,``),setTimeout(()=>this.shadow.getElementById(`search`).focus(),50)}close(){this.removeAttribute(`open`)}_buildCatBar(){let e=this.shadow.getElementById(`cat-bar`);e.innerHTML=``;for(let t of oi){let n=document.createElement(`button`);n.className=`cat-btn${t.id===this.activeCatId?` active`:``}`,n.title=t.label,n.textContent=t.icon,n.dataset.catId=t.id,n.addEventListener(`click`,()=>{this.activeCatId=t.id,this._syncCatBar(),this._renderCategory(t.id),this.shadow.getElementById(`search`).value=``}),e.appendChild(n)}}_syncCatBar(){this.shadow.querySelectorAll(`.cat-btn`).forEach(e=>{e.classList.toggle(`active`,e.dataset.catId===this.activeCatId)})}_renderCategory(e){let t=this.shadow.getElementById(`emoji-area`);t.innerHTML=``;let n=this.inserter.getRecent();if(n.length>0){this._appendSection(t,`최근 사용`,n);let e=document.createElement(`hr`);e.className=`divider`,t.appendChild(e)}let r=oi.find(t=>t.id===e);r&&this._appendSection(t,r.label,r.emojis)}_renderSearch(e){let t=this.shadow.getElementById(`emoji-area`);t.innerHTML=``;let n=ci(e);if(n.length===0){let n=document.createElement(`div`);n.className=`no-result`,n.textContent=`"${e}"에 대한 결과가 없습니다`,t.appendChild(n)}else this._appendSection(t,`검색 결과 (${n.length})`,n)}_appendSection(e,t,n){let r=document.createElement(`div`);r.className=`section-label`,r.textContent=t,e.appendChild(r);let i=document.createElement(`div`);i.className=`emoji-grid`;for(let e of n){let t=document.createElement(`button`);t.className=`emoji-btn`,t.textContent=e,t.title=e,t.addEventListener(`click`,()=>this._pickEmoji(e)),i.appendChild(t)}e.appendChild(i)}_bind(){this.shadow.querySelector(`.x-btn`).addEventListener(`click`,()=>this.close()),this.addEventListener(`click`,e=>{e.composedPath()[0]===this&&this.close()});let e=this.shadow.getElementById(`search`);e.addEventListener(`input`,()=>{let t=e.value.trim();t?this._renderSearch(t):this._renderCategory(this.activeCatId)}),e.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.stopPropagation(),this.close())})}_pickEmoji(e){this.dispatchEvent(new CustomEvent(`poa-emoji-insert`,{bubbles:!0,composed:!0,detail:{emoji:e}})),this.shadow.getElementById(`search`).value.trim()||this._renderCategory(this.activeCatId)}},Os=`
*, *::before, *::after { box-sizing: border-box; }
:host { display: none; position: fixed; inset: 0; z-index: 9999; align-items: center; justify-content: center; }
:host([open]) { display: flex; background: rgba(0,0,0,.45); }
...
.hidden { display: none !important; }
`,ks=class extends HTMLElement{shadow;mode=`add`;editId=null;entries=[];connectedCallback(){this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Os}</style>
<div class="dlg">
  <div class="hdr">
...
`,js=class extends HTMLElement{shadow;targetInput=null;connectedCallback(){this.shadow||(this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${As}</style>${this._tpl()}`,this._bindEvents())}open(e){this.shadow||this.connectedCallback(),this.targetInput=e,this._populate(e),this.setAttribute(`open`,``)}close(){this.removeAttribute(`open`)}_tpl(){return`<div class="dlg">
  <div class="hdr">
    <h3>입력 요소 속성</h3>
    <button class="x-btn" aria-label="닫기">✕</button>
  </div>
...
`;function Ns(e){return new Date(e).toLocaleString(`ko-KR`,{month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`,second:`2-digit`})}var Ps=class extends HTMLElement{shadow;autoSave=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Ms}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog" role="dialog" aria-modal="true" aria-label="문서 이력">
    ...
    <div class="footer">
      ...
      <button class="clear-btn" id="btn-clear">이력 전체 삭제</button>
        ...
        <span class="history-time">${Ns(t.savedAt)}</span>
        <button class="restore-btn" data-id="${String(t.id)}">복원</button>`,n.querySelector(`.restore-btn`).addEventListener(`click`,()=>{this.handleRestore(t)}),e.appendChild(n)}}handleRestore(e){this.dispatchEvent(new CustomEvent(`poa-autosave-restore`,{bubbles:!0,composed:!0,detail:{html:e.html}})),this.close()}},Fs=J(Lo,14),Is=J(ja,12),Ls=J(ka,12),Rs=J(Ma,12),zs={index:`번호`,title:`제목`,updatedAt:`수정일`},Bs=`
:host { display: none; }
:host([open]) { display: block; }
...
`;function Vs(e){let t=new Date(e);return`${t.getFullYear()}.${String(t.getMonth()+1).padStart(2,`0`)}.${String(t.getDate()).padStart(2,`0`)}`}var Hs=class extends HTMLElement{shadow;onLoad=null;documents=[];filterQuery=``;sortKey=`updatedAt`;sortOrder=`desc`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Bs}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog">
    <div class="header">
      <span>문서 목록</span>
    ...
    <div class="search-wrap">
      <div class="search-field">
        <span class="search-icon">${Fs}</span>
...
`,Ws=class extends HTMLElement{shadow;mode=`title`;defaultTitle=`새 문서`;titleResolve=null;unsavedResolve=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Us}</style><div class="overlay" id="overlay"></div>`,this.shadow.getElementById(`overlay`)?.addEventListener(`click`,e=>{e.target.id===`overlay`&&this._handleCancel()}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.preventDefault(),this._handleCancel())})}showTitleInput(e){return this.mode=`title`,this.defaultTitle=e,this._render(),this.setAttribute(`open`,``),new Promise(e=>{this.titleResolve=e})}showUnsavedConfirm(){return this.mode=`unsaved`,this._render(),this.setAttribute(`open`,``),new Promise(e=>{this.unsavedResolve=e})}_render(){let e=this.shadow.getElementById(`overlay`);if(this.mode===`title`){e.innerHTML=`
<div class="dialog">
...
</div>`,this.shadow.getElementById(`btn-save`)?.addEventListener(`click`,()=>{this._close(),this.unsavedResolve?.(`save`),this.unsavedResolve=null}),this.shadow.getElementById(`btn-discard`)?.addEventListener(`click`,()=>{this._close(),this.unsavedResolve?.(`discard`),this.unsavedResolve=null}),this.shadow.getElementById(`btn-cancel`)?.addEventListener(`click`,()=>this._handleCancel())}_handleTitleSave(){let e=this.shadow.getElementById(`title-inp`).value.trim()||this.defaultTitle;this._close(),this.titleResolve?.(e),this.titleResolve=null}_handleCancel(){this._close(),this.mode===`title`?(this.titleResolve?.(null),this.titleResolve=null):(this.unsavedResolve?.(`cancel`),this.unsavedResolve=null)}_close(){this.removeAttribute(`open`)}},Gs={파일:[{keys:[`Ctrl`,`N`],desc:`새 문서`},{keys:[`Ctrl`,`O`],desc:`열기`},{keys:[`Ctrl`,`S`],desc:`저장`},{keys:[`Ctrl`,`Shift`,`S`],desc:`다른 이름으로 저장`},{keys:[`Ctrl`,`P`],desc:`인쇄`}],편집:[{keys:[`Ctrl`,`Z`],desc:`실행 취소`},{keys:[`Ctrl`,`Y`],desc:`다시 실행`},{keys:[`Ctrl`,`X`],desc:`잘라내기`},{keys:[`Ctrl`,`C`],desc:`복사`},{keys:[`Ctrl`,`V`],desc:`붙여넣기`},{keys:[`Ctrl`,`Shift`,`V`],desc:`서식 없이 붙여넣기`},{keys:[`Ctrl`,`A`],desc:`전체 선택`},{keys:[`Ctrl`,`F`],desc:`찾기`},{keys:[`Ctrl`,`H`],desc:`찾기·바꾸기`}],서식:[{keys:[`Ctrl`,`B`],desc:`굵게`},{keys:[`Ctrl`,`I`],desc:`기울임`},{keys:[`Ctrl`,`U`],desc:`밑줄`},{keys:[`Ctrl`,`Shift`,`X`],desc:`취소선`},{keys:[`Ctrl`,`L`],desc:`왼쪽 정렬`},{keys:[`Ctrl`,`E`],desc:`가운데 정렬`},{keys:[`Ctrl`,`R`],desc:`오른쪽 정렬`},{keys:[`Ctrl`,`J`],desc:`양쪽 정렬`}],표:[{keys:[`Tab`],desc:`다음 셀 이동`},{keys:[`Shift`,`Tab`],desc:`이전 셀 이동`},{keys:[`Tab`],note:`(마지막 셀)`,desc:`새 행 추가`}],보기:[{keys:[`F11`],desc:`전체화면`},{keys:[`Ctrl`,`+`],desc:`확대`},{keys:[`Ctrl`,`-`],desc:`축소`},{keys:[`Ctrl`,`0`],desc:`100% 초기화`}]},Ks=[`파일`,`편집`,`서식`,`표`,`보기`];function qs(e,t){return`${e.map(e=>`<kbd>${e}</kbd>`).join(`<span class="plus">+</span>`)}${t?`<span class="sc-note">&nbsp;${t}</span>`:``}`}var Js=`
:host { display: none; }
:host([open]) { display: block; }
...
`,Ys=class extends HTMLElement{shadow;activeTab=`파일`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Js}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog">
    <div class="header">
      <span>단축키</span>
...
</div>`,this.shadow.getElementById(`btn-close`)?.addEventListener(`click`,()=>this._close()),this.shadow.getElementById(`backdrop`)?.addEventListener(`click`,e=>{e.target.id===`backdrop`&&this._close()}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&this._close()}),this.shadow.getElementById(`tabs`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-tab]`);t&&(this.activeTab=t.dataset.tab,this._renderTabs(),this._renderContent())})}open(){this.activeTab=`파일`,this.setAttribute(`open`,``),this._renderTabs(),this._renderContent(),requestAnimationFrame(()=>this.shadow.getElementById(`btn-close`)?.focus())}_renderTabs(){this.shadow.getElementById(`tabs`).innerHTML=Ks.map(e=>`<button class="tab-btn${e===this.activeTab?` active`:``}" data-tab="${e}">${e}</button>`).join(``)}_renderContent(){this.shadow.getElementById(`content`).innerHTML=Gs[this.activeTab].map(({keys:e,note:t,desc:n})=>`
<div class="sc-row">
  <div class="sc-keys">${qs(e,t)}</div>
  <span class="sc-desc">${n}</span>
</div>`).join(``)}_close(){this.removeAttribute(`open`)}},$={type:J(rs,16),bold:J(Ta,16),clipboard:J(Ia,16),search:J(Lo,16),table:J(Zo,16),chevron:J(Aa,16),merge:J(Jo,16),rows:J(No,16),image:J(to,16),maximize:J(go,16),imagePlus:J(eo,16),formInput:J(Ao,16),list:J(ho,16),user:J(os,16)},Xs={기본편집:[{icon:$.type,title:`텍스트 입력`,body:[`에디터 영역을 클릭하면 텍스트를 입력할 수 있습니다.`,`폰트, 크기, 색상 등 서식은 툴바에서 변경하세요.`]},{icon:$.bold,title:`서식 적용`,body:[`텍스트를 선택한 후 툴바 버튼을 클릭하거나 단축키(Ctrl+B, Ctrl+I 등)를 사용하세요.`]},{icon:$.clipboard,title:`복사·붙여넣기`,body:[`외부에서 복사한 내용을 붙여넣을 때 Ctrl+Shift+V 를 사용하면 서식 없이 붙여넣기됩니다.`],tip:`Ctrl+Shift+V 로 서식 없이 붙여넣기`},{icon:$.search,title:`찾기·바꾸기`,body:[`Ctrl+F 로 찾기, Ctrl+H 로 찾기·바꾸기를 열 수 있습니다.`]}],표:[{icon:$.table,title:`표 삽입`,body:[`삽입 탭 → 표 삽입 버튼을 클릭하면 그리드에서 행/열 수를 선택하여 삽입합니다.`]},{icon:$.chevron,title:`셀 이동`,body:[`Tab 키로 다음 셀, Shift+Tab 으로 이전 셀로 이동합니다.`,`마지막 셀에서 Tab 을 누르면 새 행이 자동 추가됩니다.`]},{icon:$.merge,title:`셀 병합·분할`,body:[`표 탭에서 셀 병합/셀 나누기 버튼을 사용하세요.`]},{icon:$.rows,title:`행·열 관리`,body:[`표 탭에서 행/열 삽입·삭제가 가능합니다.`]}],이미지:[{icon:$.image,title:`이미지 삽입`,body:[`삽입 탭 → 이미지 버튼으로 파일을 업로드하거나 URL로 이미지를 삽입할 수 있습니다.`]},{icon:$.maximize,title:`크기 조절`,body:[`삽입된 이미지를 클릭하면 크기 조절 핸들이 표시됩니다.`,`드래그로 크기를 변경하세요.`]},{icon:$.imagePlus,title:`이미지 편집`,body:[`편집 탭 → 이미지 편집에서 자르기, 회전, 반전이 가능합니다.`]}],양식필드:[{icon:$.formInput,title:`필드 삽입`,body:[`삽입 탭 → 양식 필드를 클릭하면 텍스트, 숫자, 날짜 등 다양한 필드를 삽입할 수 있습니다.`]},{icon:$.list,title:`필드 유형`,body:[`텍스트: 일반 문자 입력`,`숫자: 숫자 입력 (천 단위 구분, 원화 등 포맷 지원)`,`날짜: 날짜 선택 (다양한 포맷 지원)`,`계약 정보: 계약명, 계약일, 금액 등 전문 필드`]},{icon:$.user,title:`사용자 모드`,body:[`기타 탭 → 사용자 모드로 보기를 클릭하면 양식 필드만 입력 가능한 사용자 전용 화면으로 전환됩니다.`]}]},Zs=[`기본편집`,`표`,`이미지`,`양식필드`],Qs=`
:host { display: none; }
:host([open]) { display: block; }
...
`,$s=class extends HTMLElement{shadow;activeTab=`기본편집`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Qs}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog">
    <div class="header">
      <span>사용자 가이드</span>
...
</div>`,this.shadow.getElementById(`btn-close`)?.addEventListener(`click`,()=>this._close()),this.shadow.getElementById(`backdrop`)?.addEventListener(`click`,e=>{e.target.id===`backdrop`&&this._close()}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&this._close()}),this.shadow.getElementById(`tabs`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-tab]`);t&&(this.activeTab=t.dataset.tab,this._renderTabs(),this._renderContent())})}open(){this.activeTab=`기본편집`,this.setAttribute(`open`,``),this._renderTabs(),this._renderContent(),requestAnimationFrame(()=>this.shadow.getElementById(`btn-close`)?.focus())}_renderTabs(){this.shadow.getElementById(`tabs`).innerHTML=Zs.map(e=>`<button class="tab-btn${e===this.activeTab?` active`:``}" data-tab="${e}">${e}</button>`).join(``)}_renderContent(){let e=Xs[this.activeTab],t=e.length-1;this.shadow.getElementById(`content`).innerHTML=e.map((e,n)=>{let r=e.body.map(e=>`<p class="body-text">${e}</p>`).join(``),i=e.tip?`<div class="tip">${e.tip}</div>`:``;return`
<div class="section${n<t?` has-divider`:``}">
  <h3 class="sec-title">
    <span class="sec-icon">${e.icon}</span>${e.title}
  </h3>
  ${r}${i}
</div>`}).join(``)}_close(){this.removeAttribute(`open`)}},ec=[{id:`file`,label:`파일`},{id:`edit`,label:`편집`},{id:`insert`,label:`삽입`},{id:`view`,label:`보기`},{id:`table`,label:`표`},{id:`format`,label:`서식`},{id:`misc`,label:`기타`},{id:`help`,label:`도움말`}],tc=`
:host { display: block; }
.menubar {
...
`,nc=new Set([`edit`,`insert`,`table`,`format`,`misc`]),rc=class extends HTMLElement{shadow;_activeTab=`edit`;_userMode=!1;_fsHandler=null;busHandler=({tab:e})=>{this._activeTab=e,this.updateActive()};constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){let e=ec.map(e=>`<button class="tab${e.id===this._activeTab?` active`:``}" data-tab="${e.id}">${e.label}</button>`).join(``);this.shadow.innerHTML=`<style>${tc}</style><div class="menubar">${e}</div>`;let t=this.shadow.querySelector(`.menubar`),n=document.createElement(`div`);n.className=`user-area`,n.id=`user-area`;let r=document.createElement(`span`);r.style.cssText=`display:contents`,r.innerHTML=Z.userCircle,n.appendChild(r);let i=document.createElement(`span`);i.className=`user-name-text`,i.id=`user-name-text`,i.textContent=Ue(),n.appendChild(i);let a=Fi();if(a===`admin`){let e=document.createElement(`span`);e.className=`admin-badge`,e.textContent=`관리자`,n.appendChild(e)}a===`write`&&this._applyWriteMode();let o=document.createElement(`div`);o.className=`user-dropdown hidden`,o.id=`user-dropdown`;let s=document.createElement(`div`);s.className=`user-dropdown-label`,s.textContent=`사용자 이름`;let c=document.createElement(`input`);c.type=`text`,c.id=`user-name-input`,c.placeholder=`사용자 이름`;let l=document.createElement(`button`);l.className=`user-dropdown-btn`,l.id=`user-name-confirm`,l.textContent=`확인`,o.append(s,c,l),n.appendChild(o),t.appendChild(n);let u=document.createElement(`button`);u.className=`fullscreen-btn`,u.title=`전체화면으로 보기`;let d=document.createElement(`span`);d.style.cssText=`display:contents`,d.innerHTML=Z.maximize2_14,u.appendChild(d),t.appendChild(u);let f=()=>{let e=!!document.fullscreenElement;d.innerHTML=e?Z.minimize2_14:Z.maximize2_14,u.title=e?`전체화면 종료`:`전체화면으로 보기`};this._fsHandler=f,document.addEventListener(`fullscreenchange`,this._fsHandler),u.addEventListener(`click`,()=>{document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen().catch(()=>{window.parent.postMessage({type:`POA_FULLSCREEN_REQUEST`,enabled:!0},`*`)})}),this.shadow.querySelectorAll(`.tab`).forEach(e=>{e.addEventListener(`mousedown`,t=>{t.preventDefault(),N.emit(P.MENUBAR_CHANGE,{tab:e.dataset.tab})})}),this._bindUserArea(),N.on(P.MENUBAR_CHANGE,this.busHandler)}disconnectedCallback(){N.off(P.MENUBAR_CHANGE,this.busHandler),this._fsHandler&&=(document.removeEventListener(`fullscreenchange`,this._fsHandler),null)}_bindUserArea(){let e=this.shadow.getElementById(`user-area`),t=this.shadow.getElementById(`user-dropdown`),n=this.shadow.getElementById(`user-name-input`),r=this.shadow.getElementById(`user-name-confirm`),i=this.shadow.getElementById(`user-name-text`);e.addEventListener(`mousedown`,e=>{if(e.preventDefault(),e.target.closest(`#user-dropdown`))return;let r=t.classList.contains(`hidden`);t.classList.toggle(`hidden`,!r),r&&(n.value=Ue(),setTimeout(()=>{n.focus(),n.select()},50))});let a=()=>{let e=n.value.trim()||`사용자`;We(e),i.textContent=e,t.classList.add(`hidden`)};r.addEventListener(`mousedown`,e=>{e.preventDefault(),a()}),n.addEventListener(`keydown`,e=>{e.key===`Enter`&&a()}),document.addEventListener(`click`,n=>{e.contains(n.target)||t.classList.add(`hidden`)})}applyUserMode(){this._userMode=!0,nc.has(this._activeTab)&&N.emit(P.MENUBAR_CHANGE,{tab:`file`}),this._applyUserModeStyles()}_applyUserModeStyles(){this._userMode&&this.shadow.querySelectorAll(`.tab`).forEach(e=>{let t=e.dataset.tab;nc.has(t)&&(e.dataset.userDisabled=`true`)})}_applyWriteMode(){this.shadow.querySelectorAll(`.tab`).forEach(e=>{e.dataset.tab===`misc`&&(e.style.display=`none`)}),this._activeTab===`misc`&&N.emit(P.MENUBAR_CHANGE,{tab:`edit`})}updateActive(){this.shadow.querySelectorAll(`.tab`).forEach(e=>{e.classList.toggle(`active`,e.dataset.tab===this._activeTab)})}},ic={text:Z.fieldText,number:Z.fieldNumber,date:Z.fieldDate};function ac(){let e=t(),n=Array.from(new Set(e.map(e=>e.category))),r=[],i=!0;for(let t of n){let n=e.filter(e=>e.category===t);if(n.length!==0){i||r.push([``,`__sep__`]),r.push([t,`__cat__`]),i=!1;for(let e of n)r.push([e.label,`insert:field`,e.id,ic[e.type]])}}return r}var oc={file:[[[`새 문서`,`file:new`],[`열기`,`file:open`],[`저장`,`file:save`],[`다른 이름으로 저장`,`file:saveas`]],[[`인쇄`,`file:print`],[`환경설정`,`settings`],[`문서 이력`,`file:history`]]],edit:[[[`잘라내기`,`edit:cut`],[`복사`,`edit:copy`],[`붙여넣기`,`edit:paste`],[`서식 없이 붙여넣기`,`edit:paste-plain`]],[[`찾기·바꾸기`,`find-replace`],[`이미지 편집`,`edit:image-edit`],[`전체 선택`,`edit:select-all`]]],insert:[[[`이미지`,`image`]],[[`하이퍼링크`,`insert:link`],[`책갈피`,`insert:bookmark`]],[[`서명`,`insert:signature`],[`이모지`,`insert:emoji`]],[[`툴팁`,`insert:tooltip`],[`툴팁 관리`,`insert:tooltip-list`]],[[`날짜·시간`,`insert:datetime`],[`가로줄`,`insert:hr`],[`기호`,`insert:symbol`],[`페이지 구분선`,`insert:pagebreak`],[`템플릿`,`misc:template`]],[[`폼 컨트롤`,`misc:form`]],[{dropdown:!0,id:`doc-field`,label:`양식 필드`,iconKey:`formField`,items:ac()}]],view:[[[`디자인`,`view:design`],[`HTML`,`view:html`],[`미리보기`,`view:preview`],[`텍스트`,`view:text`],[`페이지`,`view:page`]],[[`전체화면`,`view:fullscreen`],[`눈금자`,`view:ruler`],[`그리드`,`view:grid`],[`숨김 테두리`,`view:hidden-border`]]],table:[[[`표 삽입`,`table`],[`표 속성`,`table:table-props`],[`셀 속성`,`table:cell-props`]],[[`셀 병합`,`table:merge`],[`셀 나누기`,`table:split-cell`]],[[`위에 행 삽입`,`table:row-above`],[`아래에 행 삽입`,`table:row-below`],[`왼쪽에 열 삽입`,`table:col-left`],[`오른쪽에 열 삽입`,`table:col-right`]],[[`행 삭제`,`table:row-delete`],[`열 삭제`,`table:col-delete`],[`표 삭제`,`table:delete`]],[[`표 왼쪽 정렬`,`table:align-left`],[`표 가운데 정렬`,`table:align-center`],[`표 오른쪽 정렬`,`table:align-right`]]],format:[[[`서식 복사`,`format:painter-copy`],[`서식 붙여넣기`,`format:painter-paste`],[`서식 제거`,`format:clear`]],[[`글머리 기호`,`format:ul`],[`글머리 번호`,`format:ol`]],[[`위 첨자`,`format:sup`],[`아래 첨자`,`format:sub`]]],misc:[[[`웹 접근성 체크`,`misc:a11y`],[`개인정보 체크`,`misc:privacy`]],[[`계산식`,`misc:calc`]],[[`사용자 모드로 보기`,`misc:user-mode`]]],help:[[[`단축키`,`help:shortcuts`],[`사용자 가이드`,`help:guide`]]]},sc=new Set([`view:design`,`view:html`,`view:preview`,`view:text`,`view:page`]),cc=`
:host {
  display: block;
  --icon-color:         #374151;
...
`,lc=new Set([`file:new`,`file:open`,`settings`]),uc=class extends HTMLElement{shadow;activeTab=`edit`;activeViewMode=`design`;outsideHandler=null;userMode=!1;busHandler=({tab:e})=>{this.activeTab=e,this.render()};viewHandler=({mode:e})=>{this.activeViewMode=e,this.activeTab===`view`&&this.render()};constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.render(),N.on(P.MENUBAR_CHANGE,this.busHandler),N.on(P.VIEW_CHANGE,this.viewHandler)}disconnectedCallback(){N.off(P.MENUBAR_CHANGE,this.busHandler),N.off(P.VIEW_CHANGE,this.viewHandler),this.closeAllDropdowns()}render(){this.closeAllDropdowns();let e=Fi()===`write`,t=oc[this.activeTab]??[];e&&this.activeTab===`insert`&&(t=t.map(e=>e.filter(e=>e===null?!0:`dropdown`in e&&e.dropdown?e.id!==`doc-field`:!0)).filter(e=>e.length>0));let n=t,r=[];for(let e=0;e<n.length;e++){e>0&&r.push(`<div class="sep"></div>`),r.push(`<div class="group">`);for(let t of n[e])if(t===null)r.push(`<div class="sep" style="margin:0 2px;"></div>`);else if(`dropdown`in t&&t.dropdown){let e=(t.id===`doc-field`?ac():t.items).map(([e,t,n,r])=>t===`__sep__`?`<hr class="drop-sep">`:t===`__cat__`?`<div class="drop-cat">${e}</div>`:`<div class="drop-item" data-action="${t}"${n?` data-value="${n}"`:``}>${r?`<span class="type-icon">${r}</span>`:``}${e}</div>`).join(``),n=t.iconKey?Z[t.iconKey]??``:``,i=n?`${n}<span>${t.label}</span>`:t.label;r.push(`<div class="dropdown-wrap" id="dd-${t.id}"><button class="btn dropdown-toggle" data-dropdown-id="${t.id}">${i}</button><div class="dropdown-menu" id="menu-${t.id}">${e}</div></div>`)}else{let[e,n,i,a]=t,o=i?` data-value="${i}"`:``,s=sc.has(n)&&n===`view:${this.activeViewMode}`?` active`:``,c=a??e,l=ls[n],u=l?Z[l]:``,d=u?`${u}<span>${e}</span>`:e;r.push(`<button class="btn${s}" data-action="${n}"${o} data-tip="${c}">${d}</button>`)}r.push(`</div>`)}this.shadow.innerHTML=`<style>${cc}</style><div class="ctx-bar">${r.join(``)}</div>`,this.userMode&&this._applyUserModeButtons(),this.shadow.querySelector(`.ctx-bar`).addEventListener(`mousedown`,e=>{let t=e.target,n=t.closest(`.dropdown-toggle`);if(n){let t=n.dataset.dropdownId;if(!t)return;e.preventDefault();let r=this.shadow.getElementById(`menu-${t}`);if(!r)return;let i=r.classList.contains(`open`);this.closeAllDropdowns(),i||(r.classList.add(`open`),this.setupOutsideClick(t));return}let r=t.closest(`.drop-item`);if(r){let t=r.dataset.action,n=r.dataset.value;if(!t)return;e.preventDefault(),this.closeAllDropdowns(),this.dispatchEvent(new CustomEvent(`poa-action`,{bubbles:!0,composed:!0,detail:{type:t,value:n}}));return}let i=t.closest(`.btn`);if(!i||i.disabled)return;let a=i.dataset.action;if(!a)return;e.preventDefault();let o=i.dataset.value;this.dispatchEvent(new CustomEvent(`poa-action`,{bubbles:!0,composed:!0,detail:{type:a,value:o}}))})}closeAllDropdowns(){this.shadow.querySelectorAll(`.dropdown-menu.open`).forEach(e=>e.classList.remove(`open`)),this.outsideHandler&&=(document.removeEventListener(`mousedown`,this.outsideHandler),null)}setupOutsideClick(e){this.outsideHandler=t=>{let n=this.shadow.getElementById(`dd-${e}`);n&&(t.composedPath().includes(n)||this.closeAllDropdowns())},document.addEventListener(`mousedown`,this.outsideHandler)}applyUserMode(){this.userMode=!0,this._applyUserModeButtons()}_applyUserModeButtons(){this.shadow.querySelectorAll(`.btn[data-action]`).forEach(e=>{let t=e.dataset.action??``;lc.has(t)&&(e.disabled=!0,e.title=(e.title||e.textContent?.trim()||``)+` (사용자 모드에서 비활성화)`)})}},dc=[{label:`기본`,value:`inherit`},{label:`Noto Sans KR`,value:`Noto Sans KR, Roboto, sans-serif`},{label:`맑은 고딕`,value:`맑은 고딕, sans-serif`},{label:`나눔고딕`,value:`나눔고딕, sans-serif`},{label:`나눔명조`,value:`나눔명조, serif`},{label:`굴림`,value:`굴림, sans-serif`},{label:`돋움`,value:`돋움, sans-serif`},{label:`바탕`,value:`바탕, serif`},{label:`궁서`,value:`궁서, serif`},{label:`Arial`,value:`Arial, sans-serif`},{label:`Times New Roman`,value:`Times New Roman, serif`},{label:`Courier New`,value:`Courier New, monospace`},{label:`Georgia`,value:`Georgia, serif`},{label:`Verdana`,value:`Verdana, sans-serif`},{label:`Tahoma`,value:`Tahoma, sans-serif`}],fc=[`8`,`9`,`10`,`11`,`12`,`14`,`16`,`18`,`20`,`24`,`28`,`32`,`36`,`48`,`72`],pc=[`1`,`1.2`,`1.4`,`1.5`,`1.6`,`1.8`,`2.0`,`2.5`],mc=[`0px`,`0.5px`,`1px`,`1.5px`,`2px`,`3px`,`4px`],hc=`
:host {
  display: block;
  --icon-color:         #374151;
  --icon-hover-bg:      #F3F4F6;
...
<div class="bar">
  ...
  ${Ri()?`<span class="mode-label">양식 입력 모드</span>`:``}
  <div class="sep"></div>
...
`,Cc=class extends HTMLElement{shadow;editor=new $i;originalSrc=``;currentDataUrl=``;busy=!1;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Sc}</style>
<div class="backdrop" id="backdrop">
...
`;function Tc(e){return e<1024*1024?`${(e/1024).toFixed(1)}KB`:`${(e/1024/1024).toFixed(1)}MB`}var Ec=class extends HTMLElement{shadow;uploader=new na;uploadConfig=null;selectedFiles=[];busy=!1;onErrorFn=null;setOnError(e){this.onErrorFn=e}constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${wc}</style>
<div class="backdrop" id="backdrop">
...
.grid-picker {
  display: grid;
  grid-template-columns: repeat(${Dc}, ${kc}px);
  gap: ${Ac}px;
  cursor: crosshair;
}
.gc {
  width: ${kc}px; height: ${kc}px;
...
`,Mc=class e extends HTMLElement{shadow;selectedPreset=`border-all`;gridCells=[];constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${jc}</style>
<div class="overlay" id="overlay">
  <div class="dialog" role="dialog" aria-modal="true">
    <div class="dlg-hd">
...
`,Fc=class extends HTMLElement{shadow;bookmarks=[];editingId=null;editingAnchor=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Pc}</style>
<div class="backdrop" id="backdrop">
  <div class="dialog" role="dialog" aria-modal="true" aria-label="링크 삽입">
    ...
    `).join(``),this.refreshDatetimePreviews(),e.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preset]`);if(!t)return;let n=Nc[parseInt(t.dataset.preset??`0`)]?.fn()??``;this.dispatch(`poa-datetime-insert`,{text:n}),this.close()}))}refreshDatetimePreviews(){Nc.forEach((e,t)=>{let n=this.shadow.getElementById(`dt-prev-${t}`);n&&(n.textContent=e.fn())})}dispatch(e,t){this.dispatchEvent(new CustomEvent(e,{bubbles:!0,composed:!0,detail:t}))}},Ic=`
:host { display: none; }
:host([open]) {
  display: flex; align-items: center; gap: 6px;
...
`,Lc=class extends HTMLElement{shadow;img=null;aspectLocked=!1;naturalW=0;naturalH=0;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Ic}</style>
<label>너비</label>
...
`,Bc=class extends HTMLElement{shadow;resolvePromise=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Rc}</style>${zc}`,this.bindEvents()}show(e,t=`확인`){let n=this.shadow.getElementById(`dlg-title`),r=this.shadow.getElementById(`dlg-message`);return n&&(n.textContent=t),r&&(r.textContent=e),this.setAttribute(`open`,``),new Promise(e=>{this.resolvePromise=e})}close(e){this.removeAttribute(`open`),this.resolvePromise?.(e),this.resolvePromise=null}bindEvents(){this.shadow.getElementById(`btn-ok`)?.addEventListener(`click`,()=>this.close(!0)),this.shadow.getElementById(`btn-cancel`)?.addEventListener(`click`,()=>this.close(!1)),this.shadow.getElementById(`overlay`)?.addEventListener(`click`,e=>{e.target.id===`overlay`&&this.close(!1)}),this.addEventListener(`keydown`,e=>{e.key===`Escape`&&(e.preventDefault(),this.close(!1))})}},Vc={error:`●`,warning:`▲`,info:`ℹ`},Hc={error:`#DC2626`,warning:`#D97706`,info:`#2563EB`},Uc=`
:host { display: none; }
:host([open]) { display: block; }
...
`,Wc=class extends HTMLElement{shadow;issues=[];contentEl=null;rerunFn=null;currentFilter=`all`;constructor(){super(),this.shadow=this.attachShadow({mode:`open`})}connectedCallback(){this.shadow.innerHTML=`<style>${Uc}</style>
<div class="overlay" id="overlay">
  <div class="dialog">
    <div class="header">
      <h2>웹 접근성 검사 결과</h2>
...
`,Yc=class extends HTMLElement{shadow;items=[];filteredType=`all`;onModified=null;onConfirmFn=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Jc}</style><slot></slot>`}setup(e,t){this.onModified=e,this.onConfirmFn=t}startLoading(){this.setAttribute(`open`,``),this.shadow.innerHTML=`<style>${Jc}</style>
<div class="overlay">
  <div class="dialog">
    <div class="header">
      <h2>개인정보 검사</h2>
...
<div class="card" data-level="${e.riskLevel}" data-id="${e.id}">
  <div class="card-header" data-toggle="${e.id}">
    <span class="card-icon">${Kc[e.riskLevel]}</span>
    <span class="card-label">${e.label}</span>
    <span class="card-risk">${qc[e.riskLevel]}</span>
    <span class="card-toggle">∨</span>
  </div>
  <div class="card-body" id="body-${e.id}">
    <div class="raw-text">"${Xc(e.raw)}"</div>
...
`,Qc=class extends HTMLElement{shadow;currentTable=null;currentRow=1;currentCol=1;pickBar=null;constructor(){super(),this.shadow=this.attachShadow({mode:`open`}),this.shadow.innerHTML=`<style>${Zc}</style>`}open(e){this.currentTable=e.table,this.currentRow=e.cellRow,this.currentCol=e.cellCol,this.setAttribute(`open`,``),this.render(e.existingFormula,e.initialRange)}close(){this.removeAttribute(`open`),this.exitPickMode(),this.currentTable=null}applyRange(e,t,n,r){this.exitPickMode();let i=e=>this.shadow.getElementById(e),a=(e,t)=>{let n=i(e);n&&(n.value=String(t))};a(`sr`,e),a(`sc`,t),a(`er`,n),a(`ec`,r)}render(e,t){let n=e?.fn??`SUM`,r=e?.format??`integer`,i=e?.decimalPlaces??2,[a,o,s,c]=e?.range??t??[1,1,1,1],l=e?.targetRow??this.currentRow,u=e?.targetCol??this.currentCol,d=!e||e.targetRow===this.currentRow&&e.targetCol===this.currentCol,f=e?.style?.backgroundColor??`#ffffff`,p=e?.style?.color??`#000000`,m=(e,t)=>`<label><input type="radio" name="fn" value="${e}"${n===e?` checked`:``}> ${e} (${t})</label>`,h=(e,t)=>`<label><input type="radio" name="format" value="${e}"${r===e?` checked`:``}> ${t}</label>`;this.shadow.innerHTML=`<style>${Zc}</style>
<div class="overlay" id="overlay">
  <div class="dialog" id="dialog">
    <div class="header">
      <h2>계산식 설정</h2>
    ...
    <div class="footer">
      ...
      ">취소</button>`,document.body.appendChild(e),this.pickBar=e,e.querySelector(`#cancel-pick`)?.addEventListener(`click`,()=>this.exitPickMode()),this.dispatchEvent(new CustomEvent(`poa-formula-start-pick`,{bubbles:!0}))}exitPickMode(){this.shadow.getElementById(`overlay`)?.classList.remove(`pick-mode`),this.pickBar?.remove(),this.pickBar=null}};customElements.get(`poa-menubar`)||customElements.define(`poa-menubar`,rc),customElements.get(`poa-context-toolbar`)||customElements.define(`poa-context-toolbar`,uc),customElements.get(`poa-toolbar`)||customElements.define(`poa-toolbar`,_c),customElements.get(`poa-status-bar`)||customElements.define(`poa-status-bar`,vc),customElements.get(`poa-settings-dialog`)||customElements.define(`poa-settings-dialog`,mn),customElements.get(`poa-file-manager-dialog`)||customElements.define(`poa-file-manager-dialog`,Ps),customElements.get(`poa-find-replace-dialog`)||customElements.define(`poa-find-replace-dialog`,xc),customElements.get(`poa-image-edit-dialog`)||customElements.define(`poa-image-edit-dialog`,Cc),customElements.get(`poa-image-dialog`)||customElements.define(`poa-image-dialog`,Ec),customElements.get(`poa-table-dialog`)||customElements.define(`poa-table-dialog`,Mc),customElements.get(`poa-cell-split-dialog`)||customElements.define(`poa-cell-split-dialog`,ia),customElements.get(`poa-link-dialog`)||customElements.define(`poa-link-dialog`,Fc),customElements.get(`poa-image-toolbar`)||customElements.define(`poa-image-toolbar`,Lc),customElements.get(`poa-confirm-dialog`)||customElements.define(`poa-confirm-dialog`,Bc),customElements.get(`poa-accessibility-dialog`)||customElements.define(`poa-accessibility-dialog`,Wc),customElements.get(`poa-privacy-dialog`)||customElements.define(`poa-privacy-dialog`,Yc),customElements.get(`poa-formula-dialog`)||customElements.define(`poa-formula-dialog`,Qc),customElements.get(`poa-video-dialog`)||customElements.define(`poa-video-dialog`,oa),customElements.get(`poa-form-control-dialog`)||customElements.define(`poa-form-control-dialog`,fa),customElements.get(`poa-template-tree`)||customElements.define(`poa-template-tree`,fs),customElements.get(`poa-template-dialog`)||customElements.define(`poa-template-dialog`,gs),customElements.get(`poa-signature-dialog`)||customElements.define(`poa-signature-dialog`,Ts),customElements.get(`poa-emoji-dialog`)||customElements.define(`poa-emoji-dialog`,Ds),customElements.get(`poa-tooltip-dialog`)||customElements.define(`poa-tooltip-dialog`,ks),customElements.get(`poa-input-property-dialog`)||customElements.define(`poa-input-property-dialog`,js),customElements.get(`poa-document-list-dialog`)||customElements.define(`poa-document-list-dialog`,Hs),customElements.get(`poa-doc-title-dialog`)||customElements.define(`poa-doc-title-dialog`,Ws),customElements.get(`poa-shortcuts-dialog`)||customElements.define(`poa-shortcuts-dialog`,Ys),customElements.get(`poa-user-guide-dialog`)||customElements.define(`poa-user-guide-dialog`,$s),customElements.get(`poa-editor`)||customElements.define(`poa-editor`,Qi);var{width:$c,height:el}=Hi(),tl=document.getElementById(`editor`);tl&&($c!==`100%`&&(tl.style.width=$c),el!==`100%`&&(tl.style.height=el,tl.style.flex=`none`));var nl=J(Wa,36),rl=J(ss,14),il=J(os,14);function al(e){return O.sanitize(e,{USE_PROFILES:{svg:!0,svgFilters:!0}})}function ol(e){return e.type===`template`&&!e.isTemp&&!e.name?.startsWith(`임시_`)&&!e.name?.startsWith(`preview_`)&&!e.name?.startsWith(`__`)&&(e.name?.trim().length??0)>0&&e.content!=null}var sl=class{container;mgr;publicGrid;privateGrid;constructor(e){this.container=e,this.mgr=new _a}renderTemplateSelector(){this.container.innerHTML=``;let e=document.createElement(`div`);e.className=`user-mode-page`;let t=document.createElement(`h2`);t.className=`user-mode-title`,t.textContent=`양식 선택`,e.appendChild(t);let[n,r]=this._buildSection(`공용 양식`,rl);this.publicGrid=r,e.appendChild(n);let[i,a]=this._buildSection(`내 양식`,il);this.privateGrid=a,e.appendChild(i),this.container.appendChild(e),this._renderSections(this.mgr.getAll().filter(ol)),this._refreshFromServer()}_buildSection(e,t){let n=document.createElement(`section`);n.className=`user-mode-section`;let r=document.createElement(`h3`);r.className=`user-mode-section-header`;let i=document.createElement(`span`);i.className=`section-header-icon`,i.innerHTML=al(t);let a=document.createElement(`span`);a.textContent=e,r.appendChild(i),r.appendChild(a),n.appendChild(r);let o=document.createElement(`div`);return o.className=`user-mode-card-grid`,n.appendChild(o),[n,o]}async _refreshFromServer(){try{let e=await G.getTemplates();this._renderSections(e.filter(ol))}catch{}}_renderSections(e){let t=new Set,n=e.filter(e=>t.has(e.id)?!1:(t.add(e.id),!0)),r=new Map;for(let e of n){let t=`${e.isPublic?`pub`:`pri`}|${e.name}`,n=r.get(t);(!n||e.updatedAt>n.updatedAt)&&r.set(t,e)}let i=Array.from(r.values()),a=i.filter(e=>e.isPublic),o=i.filter(e=>!e.isPublic);this._renderCards(this.publicGrid,a,`public`,`등록된 공용 양식이 없습니다.`),this._renderCards(this.privateGrid,o,`private`,`저장된 개인 양식이 없습니다.`)}_renderCards(e,t,n,r){if(e.innerHTML=``,t.length===0){let t=document.createElement(`p`);t.className=`user-mode-empty`,t.textContent=r,e.appendChild(t)}else for(let r of t)e.appendChild(this._buildCard(r,n))}_buildCard(e,t){let n=document.createElement(`div`);n.className=`user-mode-card user-mode-card--${t}`;let r=document.createElement(`div`);r.className=`user-mode-card-icon`,r.innerHTML=al(nl);let i=document.createElement(`div`);return i.className=`user-mode-card-name`,i.textContent=e.name,n.appendChild(r),n.appendChild(i),n.addEventListener(`click`,()=>{window.location.href=Vi(e.id)}),n}static async loadTemplateById(e){if(e===`__preview__`){let e=sessionStorage.getItem(`poa-preview-html`);return e?{id:`__preview__`,type:`template`,name:`미리보기`,parentId:null,content:e,isPublic:!1,createdAt:Date.now(),updatedAt:Date.now(),order:0}:null}let t=U(e);if(t!==null)try{return await G.getTemplate(t)}catch{}return new _a().getById(e)??null}};if(Fi()===`user`){let e=zi();if(e){let t=await sl.loadTemplateById(e),n=document.getElementById(`editor`);t?.content&&n.setHTML(t.content),n.enterUserMode(t?.name??`문서`)}else document.getElementById(`editor-root`).classList.add(`hidden`),document.body.classList.add(`user-mode`),new sl(document.getElementById(`user-mode-root`)).renderTemplateSelector()}